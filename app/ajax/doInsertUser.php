...
if(isset($_GET['nm']) && isset($_GET['sn']) && isset($_GET['sx']) && isset($_GET['ns']) && isset($_GET['en']) && isset($_GET['cp']) && isset($_GET['br']) && isset($_GET['tl']) && isset($_GET['cpf']) && isset($_GET['rg']) && isset($_GET['pa']) && isset($_GET['ma']) && isset($_GET['nv']) && isset($_GET['lg']) && isset($_GET['se']) && isset($_GET['re']) && isset($_GET['pg']) && isset($_GET['rs'])){
	...
	if(!preg_match("/^[A-Za-z çéáíóúâôêãõ'\.]{2,45}$/", $_GET['nm'])){
		...
		$msgError = "<strong class=\"titleMsgError\">Pergunta secreta inv&aacute;lida!</strong><br><strong>Tamanho</strong>: m&iacute;nimo 2 e m&aacute;ximo 45 caracteres.<br><strong>Caracteres permitidos</strong>: letras, acentua&ccdil;&atilde;o, espa&ccedil;o em branco e h&iacute;fen.";
		
	}elseif(!preg_match("/^[A-Za-z0-9_-çéáíóúâôêãõ]{1,10}$/", $_GET['rs'])){
		
		$msgError = "<strong class=\"titleMsgError\">Resposta secreta inv&aacute;lida!</strong><br><strong>Tamanho</strong>: m&iacute;nimo 1 e m&aacute;ximo 10 caracteres.<br><strong>Caracteres permitidos</strong>: letras, acentua&ccdil;&atilde;o, n&uacute;meros, underline(_) e h&iacute;fen.";
	...
	require_once("config/connect.php");
		...
		$sql->bindParam(":login", $_GET['lg'], PDO::PARAM_STR);
		...
		if(!$sql->execute()){
			...
			if($obj[0]>0){
				$msgError =  "Login informado j&aacute; esta sendo utilizado!";
			}else{
				...
				$sqlPes = $pdo->prepare("INSERT INTO
										...
										VALUES
										(1, :nome, :sobrenome, :sexo, now(), :nascimento, :mae, :pai, :cpf, :rg, :telefone, :endereco, :complemento, :bairro)");
				...
				if(!$sqlPes->execute()){
					...
					$sqlUsu = $pdo->prepare("INSERT INTO
									 usuario
									 (id_pessoa,id_nivel_acesso,login,senha,status,pergunta,resposta)
									 VALUES
									 ((SELECT last_insert_id()), :id_nivel_acesso, :login, :senha, 1, :pergunta, :resposta)
									 ");
