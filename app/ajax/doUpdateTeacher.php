...
if(isset($_GET['nm']) && isset($_GET['sn']) && isset($_GET['sx']) && isset($_GET['ns']) && isset($_GET['en']) && isset($_GET['cp']) && isset($_GET['br']) && isset($_GET['tl']) && isset($_GET['cpf']) && isset($_GET['rg']) && isset($_GET['pa']) && isset($_GET['ma']) && isset($_GET['gr'])){
	...
	require_once("config/connect.php");
		...
		$sql = $pdo->prepare("SELECT
							pr.id_professor,pe.id_pessoa,a.id_atleta,pe.id_tipo
							FROM pessoa AS pe
							...
							WHERE pe.id_pessoa = :id_pessoa;
								 ");
		$sql->bindParam(":id_pessoa",$_GET['id'],PDO::PARAM_INT);
		...
		if(!$sql->execute()){
			...
			$sqlPes->bindParam(":id_pessoa",$_GET['id'],PDO::PARAM_INT);
			...
			if(!$sqlPes->execute()){
					$msgError = "Erro ao executar a query (1)".print_r($sqlPes->errorInfo());
			}else{
				unset($stmt);
				$stmt = "UPDATE 
						atleta
				...
				$sqlAtl = $pdo->prepare($stmt);			
				$sqlAtl->bindParam(":id_pessoa",$_GET['id'],PDO::PARAM_INT);
				$sqlAtl->bindParam(":id_atleta",$obj[0]->id_atleta,PDO::PARAM_INT);
				$sqlAtl->bindParam(":id_grau",$_GET['gr'],PDO::PARAM_INT);			

				if(!$sqlAtl->execute()){
					$msgError = "Erro ao executar a query (2)".print_r($stmt);
				}
			}
		}
		if($msgError != NULL){
